...
\end{split}
$$
The intuition is that $\text{openness}(\text{move}) - \text{overlap}(\text{move})$ is related to the increase in the total overlap in a board.  The actual formula is 
$$
\text{total_overlap}_p' = \text{total_overlap}_p +  \text{openness}_p(\text{move}) - \text{overlap}_p(\text{move})
$$
where $\text{total_overlap}_p$ is player $p$'s total overlap in the board before playing a move $\text{move}$ and $\text{total_overlap}_p'$ is the total overlap after playing $\text{move}$. Thus the easy player's strategy can be rewritten as 
$$
\begin{split}
\text{easy}(\text{move})
...
\end{split}
$$
The first two terms after  $\text{centrality}$ engourage the maximization of the immediate overlap of a move (exploitation), while the last two terms encourage the ai to maximize the total overlap of the board (exploration).  Thus there is a tradeoff between exploration and exploitation; exploition being 3 times more important than exploration.  In the formula for $\Delta \text{score}_p$, overlap has the largest positive coefficient and so it makes sense that the ai using this value approximation does reasonably well.  
