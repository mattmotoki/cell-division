...
The easy ai's value function is 
$$
\begin{split}
\text{easy}(\text{move}) 
...
\end{split}
$$
The intuition is that $\text{openness}(\text{move}) - \text{overlap}_p(\text{move})$ is related to the increase in the total overlap in a board.  The actual formula for the increaes in the total overlap of a board when playing $\text{move}$ is 
$$
\Delta\text{total_overlap}_p(\text{move}) = \text{openness}_p(\text{move}) - \text{overlap}_p(\text{move})
$$
The easy ai's strategy can be rewritten as 
$$
\begin{split}
\text{easy}(\text{move})
& =\text{centrality}(\text{move})\\
& \quad + 1.5 \cdot\big[ \text{overlap}_2(\text{move}) +   \text{overlap}_1(\text{move})\big] \\ 
& \quad + 0.5\cdot\big[ \Delta\text{total_overlap}_2(\text{move}) + \Delta\text{total_overlap}_1(\text{move})\big].
\end{split}
$$
The two $\text{overlap}$ terms encourage exploitation of the current move while the two $\Delta\text{total_overlap}$ terms encourage exploration of the board.  Overlap is the largest contributor to $\Delta \text{score}_p$ and so it makes sense that the ai using this value approximation does reasonably well.  

The hard ai modifies the above idea by switching $\text{overlap}$ with $\Delta \text{score}$, which after simplification yields
$$
\begin{split}
\text{hard}(\text{move}) 
& = \text{centrality}(\text{move}) + \text{openness}(\text{move})\\
& \quad +  1.5\cdot \big[ \Delta \text{score}_2(\text{move}) + \Delta \text{score}_1(\text{move}) \big]\\
& \quad -  0.5\cdot \big[ \text{overlap}_2(\text{move}) + \text{overlap}_1(\text{move}) \big]
.
\end{split}
$$
The hope is that hard ai will choose a move that balances the immediate score increases of the move with the total overlap of the resulting board.  
