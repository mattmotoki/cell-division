...
Cells are drawn as layered polygons in base R.  Before calling the `polygon` function we must create a new plot.  The `blank_plot` function creates a new empty plot and draws lines to highlight the edges of the cell which we will be at 0 and 1 in both directions. 
```{r}
blank_plot <- function() {
...
The center of mass (COM) can be calculated using a double integral, but it is easier to use the following hack.  First, save the outline of the cell as a temporary png, then read it back into R as a 3-dimensional array (two dimensions for the x and y coordinate and the third dimension for the transparency value).  The center of mass can then be obtained by taking inner products between the rows/cols of the transparency matrix and the corresponding y/x values. The `com_calc` takes in the `x` and `y` values of the cell outline and applies the aforementioned hack.
```{r}
...
\begin{split}
\text{easy}(\text{move}) 
&= \text{centrality}(\text{move}) + \text{openness}(\text{move}) \\
& \quad+ \text{overlap}_2(\text{move}) + \text{overlap}_1(\text{move}).
\end{split}
$$
The intuition is that $\text{openness}(\text{move}) - \text{overlap}_p(\text{move})$ is related to the increase in the total overlap in a board.  The actual formula is 
$$
\text{total_overlap}_p' = \text{total_overlap}_p +  \text{openness}_p(\text{move}) - \text{overlap}_p(\text{move})
$$
where $\text{total_overlap}_p$ is player $p$'s total overlap in the board before playing $\text{move}$ and $\text{total_overlap}_p'$ is the total overlap after playing $\text{move}$. Thus the easy player's strategy can be rewritten as 
$$
\begin{split}
\text{easy}(\text{move})
& =\text{centrality}(\text{move})\\
& \quad + 1.5 \cdot \text{overlap}_1(\text{move}) + 1.5 \cdot  \text{overlap}_2(\text{move}) \\ 
& \quad + 0.5\cdot \big[ \text{openness}(\text{move}) - \text{overlap}_2(\text{move}) \big] \\
& \quad + 0.5\cdot \big[ \text{openness}(\text{move}) - \text{overlap}_1(\text{move}) \big] .
\end{split}
$$
The first two terms after  $\text{centrality}$ engourage the maximization of the immediate overlap of a move (exploitation), while the last two terms encourage the ai to maximize the total overlap of the board (exploration).  Thus there is a tradeoff between exploration and exploitation, with exploition being 3 times more important than exploration.  In the formula for $\Delta \text{score}_p$, overlap has the largest positive coefficient and so it makes sense that the ai using this value approximation does reasonably well.  

The hard opponent takes the above idea further and aims at obtaining a tradeoff between immediate score increase and total "scoring potential";
$$
\begin{split}
\text{hard}(\text{move}) 
& = \text{centrality}(\text{move}) + \text{openness}(\text{move})\\
& \quad +  \Delta \text{score}_2(\text{move}) + \Delta \text{score}_1(\text{move}).
\end{split}
$$
Although this formula looks similar to $\text{easy}(\text{move})$, it does not have the same interpretation. That said, even with out any theoretical justification, the ai using this metric is generally harder to beat than the easy ai.  
...
$$
Hence, if you beat your opponent, then its skill goes up by the score difference; if you lose to your opponent, then its skill goes down in proportion to half the absolute score difference.  If the game is a draw, then the skill level stays the same.

Ignoring within-game adaptivity, when $\text{skill}=0$ you are essentially playing against the easy ai, when $\text{skill}=1$ you are playing against the hard ai, and similarly, \text{skill} $\text{skill}=0.5$ you are playing against the medium ai.
